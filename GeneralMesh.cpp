...
void General_Mesh::genCube(double half_length)
{
	Eigen::Vector3d temp(-half_length, -half_length, half_length);
	...
	temp.x() = -half_length, temp.y() = -half_length, temp.z() = -half_length;
	cube_vertex.push_back(temp);
	temp.x() = half_length;
	cube_vertex.push_back(temp);
	temp.y() = half_length;
	...
	{
		fout << "v " << cube_vertex[i].x() << " " << cube_vertex[i].y() << " " << cube_vertex[i].z() << endl;
	}
...
void General_Mesh::genCylinder()
{
	...
	for (int k = 0; k < 2; k++)
	{
		...
		for (int i = 0; i < 24; i++)
		{
			Eigen::Vector3d temp(x[i], y[i], height);
	...
	{
		fout << "v " << cylinder_vertex[i].x() << " " << cylinder_vertex[i].y() << " " << cylinder_vertex[i].z() << endl;
	}
...
void General_Mesh::genHollowCylinder(float inside_radius, float outside_radius, float height)
{
	...
	for (int i = 0; i < 2; i++)
	{
		for (int j = 0; j < 2; j++)
		{
			...
			{
				Eigen::Vector3d temp(r[i] * cos(2 * k * PI / segs), r[i] * sin(2 * k * PI / segs), h[j]);
	...
	{
		fout << "v " << hollow_cylinder_vertex[i].x() << " " << hollow_cylinder_vertex[i].y() << " " << hollow_cylinder_vertex[i].z() << endl;
	}
...
{
	float y, z;
	Eigen::Vector3d temp(thick / 2, thick / 2, thick / 2);

	temp.y() = width / 2, temp.z() = 0;
	hollow_cube_vertex.push_back(temp);
	temp.y() = width / 2, temp.z() = width / 2;
	hollow_cube_vertex.push_back(temp);
	temp.y() = -width / 2, temp.z() = width / 2;
	hollow_cube_vertex.push_back(temp);
	temp.y() = -width / 2, temp.z() = 0;
	...
	for (int i = 0; i < 24; i++)
	{
		temp.y() = radius * cos(i * PI / 12);
		temp.z() = radius * sin(i * PI / 12);
		hollow_cube_vertex.push_back(temp);
	}

	Eigen::Vector3d x_dir(1, 0, 0);
	for (int i = 0; i < 30; i++)
	{
		Eigen::Vector3d t = hollow_cube_vertex[i] - thick * x_dir;
	...
	{
		fout << "v " << hollow_cube_vertex[i].x() << " " << hollow_cube_vertex[i].y() << " " << hollow_cube_vertex[i].z() << endl;
	}
...
void General_Mesh::insert(vector<Eigen::Vector3d>& origin_vertexs, vector<v_int>& origin_faces,
...
void General_Mesh::insert(vector<Eigen::Vector3d> vertexs, vector<v_int> faces)
{
...
void General_Mesh::genResultMesh(const char* filename)
{
	...
	if (r != 0 || g != 0 || b != 0)
		...
		{
			fout << "v " << result_vertex[i].x() << " " << result_vertex[i].y() << " " << result_vertex[i].z() << " " << r << " " << g << " " << b << " " << endl;
		}
	else
		for (int i = 0; i < result_vertex.size(); i++)
		{
			fout << "v " << result_vertex[i].x() << " " << result_vertex[i].y() << " " << result_vertex[i].z() << endl;
		}
...
void General_Mesh::genResultMesh_2(vector<int> num, vector<vector<double>> colors, const char* filename)
{
	ofstream fout;
	fout.open(filename);
	int t = 0;
	...
	for (int i = 0; i < result_vertex.size(); i++)
	{
		if (i == num[t])
			t++;
		fout << "v " << result_vertex[i].x() << " " << result_vertex[i].y() << " " << result_vertex[i].z() << " " << colors[t][0] << " " << colors[t][1] << " " << colors[t][2] << " " << endl;
	}
...
//���¼�����������תƽ�Ʒ�������
vector<Eigen::Vector3d> General_Mesh::meshTrans(Eigen::Vector3d dire, vector<Eigen::Vector3d> point)
{
...
vector<Eigen::Vector3d> General_Mesh::meshScale(Eigen::Vector3d scale, vector<Eigen::Vector3d> point)
{
	vector<Eigen::Vector3d> result;
	int count = point.size();
	for (int i = 0; i < count; i++)
	{
		result.push_back(point[i].cwiseProduct(scale));
	}
	return result;
}

vector<Eigen::Vector3d> General_Mesh::meshRotate(Eigen::Vector3d z_dire, vector<Eigen::Vector3d> point)
{
	z_dire.normalize();

	Eigen::Vector3d x_dire, y_dire;

	float t = powf(z_dire.x(), 2) + powf(z_dire.y(), 2);
	...
	y_dire = z_dire.cross(x_dire);

	vector<Eigen::Vector3d> result;
	for (int i = 0; i < point.size(); i++)
	{
		Eigen::Vector3d temp = point[i].x() * x_dire + point[i].y() * y_dire + point[i].z() * z_dire;
...
vector<Eigen::Vector3d> General_Mesh::meshRotate(Eigen::Vector3d x_dire, Eigen::Vector3d z_dire, vector<Eigen::Vector3d> point)
{
	x_dire.normalize();
	z_dire.normalize();

	Eigen::Vector3d y_dire;
	y_dire = z_dire.cross(x_dire);
