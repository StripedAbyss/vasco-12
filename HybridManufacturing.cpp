	...
	double& sum_time_5)
{
	...
	if (tree_entries[now_last_node].judge_continue == false) {
		...
		vasco::mesh_validation::LoadAndCheckCurrentNodeMesh(current_file_name, current_node_mesh);	//加载当前节点对应的模型
		...
		vasco::mesh_validation::LoadAndCheckCurrentNodeMesh(current_file_name, current_node_mesh);
