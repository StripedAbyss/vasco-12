...
//      read the normal vector but ignore it.
        ...
        /////new//////
        if (model.faces_[model.faces_.size() - 1] == model.faces_[model.faces_.size() - 2] || model.faces_[model.faces_.size() - 1] == model.faces_[model.faces_.size() - 3] || model.faces_[model.faces_.size() - 2] == model.faces_[model.faces_.size() - 3]) {
            model.faces_.erase(model.faces_.begin() + model.faces_.size() - 1);
            model.faces_.erase(model.faces_.begin() + model.faces_.size() - 1);
            model.faces_.erase(model.faces_.begin() + model.faces_.size() - 1);
        }
        //////////////
