...
void Slicer::buildLayers()
{
    ...
    Katana::Instance().min_z = by_z.front().value;   //+0
    ...
    float next_layer_z = Katana::Instance().min_z + layer_height;    // layer_height/2   //Katana::Instance().min_z + layer_height+0.9
...
void Slicer::buildSegments(vector<Vertex> ori_space_points, std::map<int, int> indices_mapping,std::string mapping_file_name)
{
    ...
    int E_value = 0;
    vector<vector<Eigen::Vector3d>> real_path;
    for (unsigned int layerIndex = 0; layerIndex < Katana::Instance().layers.size(); layerIndex++)
    {
        ...
        ///////mapping
        vector<Eigen::Vector3d> current_path;
        for (int i = 0;i < layer.segments.size();i++) {
            int ori_index_1;
            int ori_index_2;
...
void Slicer::buildSegments()
{
    ...
    for (unsigned int layerIndex = 0; layerIndex < Katana::Instance().layers.size(); layerIndex++)
    {
        ...
        for (unsigned int i = 0; i < layer.triangles.size(); i++)
        {
            ...
            Katana::Instance().map_segment_triangles[layerIndex].insert({ temp_pair, layer.triangles[i] });
