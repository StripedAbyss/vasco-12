...
void Visual::insert_Line(General_Mesh& mesh, std::vector<Eigen::Vector3d> points, float radius)
{
    if (points.size() < 2)
        return;
    ...
    for (int i = 0; i < points.size() - 1; i++)
    {
        ...
        Eigen::Vector3d v0 = points[i];
        Eigen::Vector3d v1 = points[i + 1];

        Eigen::Vector3d t_dir = v1 - v0;

        Eigen::Vector3d scale(radius, radius, t_dir.norm());
        Eigen::Vector3d trans = (v0 + v1) / 2;

        vector<Eigen::Vector3d> c_vertex = mesh.meshScale(scale, mesh.cylinder_vertex);
    ...
    for (int i = 0; i < lines.size(); i++)
    {
        for (int j = 0; j < lines[i].size(); j++)
        {
            for (int k = 0; k < lines[i][j].size(); k++) {
                ...
                the_point.x() = lines[i][j][k](0,0);
                the_point.y() = lines[i][j][k](1,0);
                the_point.z() = lines[i][j][k](2,0);
...
void Visual::generateModelForRendering_2(Eigen::Vector3d lines, string file_name)
{
    ...
    the_point.y() = lines.y();
    the_point.z() = lines.z();
    Eigen::Vector3d the_point_2(0, 0, 0);
...
void Visual::generateModelForRendering_3(Eigen::Vector3d lines, string file_name, std::vector<Eigen::MatrixXd> vis_points)
{
    ...
    for (int i = 0; i < 1; i++) {
        the_point_2.x() = vis_points[i](0, 0);
        the_point_2.y() = vis_points[i](1, 0);
        the_point_2.z() = vis_points[i](2, 0);

        the_point.x() = the_point_2.x() + lines.x() * 100;
        the_point.y() = the_point_2.y() + lines.y() * 100;
        the_point.z() = the_point_2.z() + lines.z() * 100;
        points.push_back(the_point_2);
        points.push_back(the_point);
        insert_Line(mesh1, points, radius);
        points.clear();
    ...
    for (int block = 0; block < all_blocks.size(); block++) {
        ...
        for (int i = 0; i < all_blocks[block].size(); i++) {
            for (int j = 0; j < lines[all_blocks[block][i]].size(); j++) {
                Eigen::Vector3d the_point;
                the_point.x() = lines[all_blocks[block][i]][j](0, 0);
                the_point.y() = lines[all_blocks[block][i]][j](1, 0);
                the_point.z() = lines[all_blocks[block][i]][j](2, 0);
                points.push_back(the_point);
            }
            insert_Line(mesh1, points, radius);
            points.clear();
...
void Visual::generateModelForRendering_5(vector<vector<cv::Point3d>> lines, vector<vector<cv::Point3d>> line_contain,int hegiht, int num, string file_name, int index_of_pre_node,bool judge_continue_additive, int id_continue)
{
    ...
    for (int i = 0; i < lines.size(); i++) {
        ...
        for (int j = 0; j < lines[i].size(); j++) {
            Eigen::Vector3d the_point;
            the_point.x() = lines[i][j].x;
            the_point.y() = lines[i][j].y;
            the_point.z() = lines[i][j].z;
            points.push_back(the_point);
            file_txt << the_point.x() << " " << the_point.y() << " " << the_point.z() << endl;
            if (j != 0) {
                sum_E++;
                file_gcode << "G1" << " X" << the_point.x() << " Y" << the_point.y() << " Z" << the_point.z() << " E" << sum_E << endl;
            }
            else
                file_gcode << "G0" << " X" << the_point.x() << " Y" << the_point.y() << " Z" << the_point.z() << endl;
        }
        if (line_contain[i].size() != 0) {
            file_txt << line_contain[i].size() << endl;
            for (int j = 0; j < line_contain[i].size(); j++) {
                Eigen::Vector3d the_point;
                the_point.x() = line_contain[i][j].x;
                the_point.y() = line_contain[i][j].y;
                the_point.z() = line_contain[i][j].z;
                file_txt << the_point.x() << " " << the_point.y() << " " << the_point.z() << endl;
            }
        }
        insert_Line(mesh1, points, radius);
...
void Visual::generateModelForRendering_6(std::vector<cv::Point3d> orientations, string file_name)
{
    ...
    Eigen::Vector3d the_point;
    Eigen::Vector3d the_point_2;
    the_point.x() = the_point.y() = the_point.z() = 0;
    for (int i = 0; i < orientations.size(); i++) {
        the_point.x() += orientations[i].x;
        the_point.y() += orientations[i].y;
        the_point.z() += orientations[i].z;
    }
    the_point /= orientations.size();

    for (int i = 0; i < orientations.size(); i++) {
        the_point_2.x() = orientations[i].x;
        the_point_2.y() = orientations[i].y;
        the_point_2.z() = orientations[i].z;
...
void Visual::generateModelForRendering_7(cv::Point3d orientations, string file_name)
{
    ...
    the_point_2.x() = orientations.x * 10;
    the_point_2.y() = orientations.y * 10;
    the_point_2.z() = orientations.z * 10;
...
void Visual::generateModelForRendering_8(vector<vector<Eigen::Vector3d>> lines, string file_name)
{
    ...
    for (int i = 0; i < lines.size(); i++) {
        ...
        the_point.x() = lines[i][0].x();
        the_point.y() = lines[i][0].y();
        the_point.z() = lines[i][0].z();
        points.push_back(the_point);
        insert_Line(mesh1, points, radius);
        points.clear();
    }
...
void Visual::generateModelForRendering_9(vector<vector<cv::Point3d>> lines, string file_name)
{
    ...
    for (int i = 0; i < lines.size(); i++) {
        for (int j = 0; j < lines[i].size(); j++) {
            the_point_2.x() = lines[i][j].x;
            the_point_2.y() = lines[i][j].y;
            the_point_2.z() = lines[i][j].z;
    ...
    for (int t = 0; t < lines.size(); t++) {
        ...
        for (int i = 0; i < lines[t].size(); i++) {
            for (int j = 0; j < lines[t][i].size(); j++) {
                the_point.x() = lines[t][i][j].x();
                the_point.y() = lines[t][i][j].y();
                the_point.z() = lines[t][i][j].z();
                points.push_back(the_point);
            }
            the_point.x() = lines[t][i][0].x();
            the_point.y() = lines[t][i][0].y();
            the_point.z() = lines[t][i][0].z();
            points.push_back(the_point);
            insert_Line(mesh1, points, radius);
...
void Visual::generateArrows(cv::Point3d normal, string file_name)
{
    ...
    Eigen::Vector3d the_point_2;
    the_point_2.x() = normal.x * 1.8;
    the_point_2.y() = normal.y * 1.8;
    the_point_2.z() = normal.z * 1.8;
    points.push_back(the_point_2);
    insert_Line(mesh1, points, radius);

    cv::Point3d center = { the_point_2.x(), the_point_2.y(), the_point_2.z() };  // 圆心
...
void Visual::generateArrows_2(vector<Eigen::Vector3d>points_2, vector<cv::Point3d> normal, string file_name)
{
    float radius = 0.18;
    General_Mesh mesh1;
    //vector<vector<double>> colors(points_in_cell.size(), vector<double>(3));
    ...
    for (int i = 0; i < points_2.size(); i++) {
        std::vector<Eigen::Vector3d> points;
        Eigen::Vector3d the_point;
        the_point.x() = points_2[i].x();
        the_point.y() = points_2[i].y();
        the_point.z() = points_2[i].z();
        points.push_back(the_point);

        Eigen::Vector3d the_point_2;
        the_point_2.x() = normal[i].x * 1.8;
        the_point_2.y() = normal[i].y * 1.8;
        the_point_2.z() = normal[i].z * 1.8;
        points.push_back(the_point_2);
        insert_Line(mesh1, points, radius);
